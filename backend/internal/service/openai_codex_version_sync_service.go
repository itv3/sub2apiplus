...
const (
	// openAICodexVersionSyncInterval 自动发现间隔。上游客户端发版频率是天级，
	...
	// fetchLatestStableVersion）。该仓库相邻稳定版之间可能隔着 20 多个 alpha；实测 30 条里
	// 只有 2 条稳定版，第二条已排在第 26 位，因此这个页大小不能再往下调，
	// 否则整页扫不到稳定版、同步会静默停更。
	openAICodexVersionSyncPerPage = 30
	// openAICodexVersionTagPrefix 客户端 release 的 tag 前缀（形如 rust-vX.Y.Z）。
	// 同仓库还有其他组件的 tag（如 rusty-v8-*），必须按前缀过滤，否则会同步到无关版本号。
	openAICodexVersionTagPrefix = "rust-v"
)

// OpenAICodexVersionSyncService 周期性把官方 Codex 客户端的最新稳定版发现结果写入设置，
// 供兼容验证和后续画像升级使用；它不移动 ReleaseCatalog active 指针。
//
// 发现值写入 SettingKeyOpenAICodexClientVersionSynced（本服务独占写入）；管理员在面板填写的
// SettingKeyOpenAICodexClientVersion 优先级更高，因此手工候选值不会被自动发现覆盖。
