...
// officialCodex0145EndpointURLInput 是端点 URL 的唯一动态输入面。
//
// 版本画像负责 host、固定 path 与固定 query；调用方只允许填充画像明确声明的
// path/query 占位符。文件直传是唯一例外：它必须完整使用服务端返回的绝对 URL，
// 不能把签名 query 拆开后重新编码。
type officialCodex0145EndpointURLInput struct {
	PathValues  map[string]string
	QueryValues map[string]string
	ReturnedURL string
}

// officialCodex0145HeaderField 是 header 契约执行后的有序字段。
//
...
type officialCodex0145HeaderField struct {
	Slot     int
	Sequence int
	Name     string
	WireName string
...
// officialCodex0145NormalizeDerivedToolPresentation 检查第三方派生路径是否已经
// 提供 Codex 的 namespace/imagegen 形态。hosted image_generation 没有等价的
// 本地工具调度闭环，不能只改一个 type 就冒充官方行为，因此明确拒绝。
func officialCodex0145NormalizeDerivedToolPresentation(
	version string,
	...
	if !officialCodex0145ToolPresentationApplies(contract, endpointID) || payload == nil {
		return false, nil
	}
	...
	for index, rawTool := range tools {
		...
		if !object || officialCodex0145MapString(tool, "type") != contract.HostedImageGenerationType {
			continue
		}
...
// officialCodex0145ValidateToolPresentation 在最终 JSON 闭包之后执行语义校验。
// 顶层字段闭集不能识别 tools 内部的协议差异，因此这条检查仍由同一版本画像驱动，
// 不按模型名、路由名或 header 临时猜测。
func officialCodex0145ValidateToolPresentation(
	version string,
	endpointID codex0145EndpointID,
	body []byte,
	responsesLite bool,
	...
	contract := profile.ToolPresentation
	if !officialCodex0145ToolPresentationApplies(contract, endpointID) {
		return nil
	}
	...
	if err := officialCodex0145ValidateToolList(contract, payload["tools"], !responsesLite, "tools"); err != nil {
		return err
	}
	input, _ := payload["input"].([]any)
	for index, rawItem := range input {
		item, ok := rawItem.(map[string]any)
		if !ok || officialCodex0145MapString(item, "type") != contract.LiteCarrierItemType {
			continue
		}
		if err := officialCodex0145ValidateToolList(
			contract,
			item["tools"],
			responsesLite,
...
func officialCodex0145ToolPresentationApplies(
...
func officialCodex0145ValidateToolList(
	contract officialCodexToolPresentationProfile,
	rawTools any,
	...
	for index, rawTool := range tools {
		...
		toolType := officialCodex0145MapString(tool, "type")
		...
		if toolType != contract.NamespaceType || officialCodex0145MapString(tool, "name") != contract.NamespaceName {
			continue
		}
		if !allowImageNamespace {
			return fmt.Errorf("Codex 0.145.0 %s[%d] 的 image_gen namespace 位于错误载体", location, index)
		}
		if err := officialCodex0145ValidateImageNamespaceDefinition(contract, tool); err != nil {
...
func officialCodex0145ValidateImageNamespaceDefinition(
	contract officialCodexToolPresentationProfile,
	...
	if officialCodex0145MapString(namespace, "description") == "" {
		return errors.New("namespace description 为空")
	}
	tools, _ := namespace["tools"].([]any)
	for _, rawTool := range tools {
		tool, ok := rawTool.(map[string]any)
		if ok && officialCodex0145MapString(tool, "type") == contract.FunctionType &&
			officialCodex0145MapString(tool, "name") == contract.FunctionName {
			for _, field := range contract.FunctionRequiredFields {
				if _, exists := tool[field]; !exists {
					return fmt.Errorf("缺少 imagegen 字段 %s", field)
				}
			}
			if officialCodex0145MapString(tool, "description") == "" {
...
func officialCodex0145MapString(values map[string]any, name string) string {
	value, _ := values[name].(string)
	return strings.TrimSpace(value)
}

// officialCodex0145ResolveTLSProfile 把不可变版本画像编译成一次调用独占的 TLS
// 与 HTTP/1.1 wire 画像。返回值中的所有切片都是新分配的，调用方修改它不会污染
// 后续解析，也不会改变版本画像摘要。
func officialCodex0145ResolveTLSProfile(version, transportID string) (*tlsfingerprint.Profile, error) {
	versionProfile, err := resolveCodex0145VersionProfile(version)
	if err != nil {
		return nil, err
	}
	...
	if transport == nil {
		return nil, fmt.Errorf("Codex %s 不支持传输画像：%q", version, transportID)
	}

	rules, err := officialCodex0145CompileH1Rules(versionProfile, *transport)
	if err != nil {
		return nil, err
	}
...
// officialCodex0145ResolveEndpointTLSProfile 先严格解析端点，再通过端点声明的
// TransportID 编译传输。未知端点不会退回默认 HTTP 画像。
func officialCodex0145ResolveEndpointTLSProfile(
	version string,
	endpointID codex0145EndpointID,
	...
	return officialCodex0145ResolveTLSProfile(version, endpoint.TransportID)
}

// officialCodex0145ResolveEndpointTLSProfileForURL 为带 path 占位符或服务端返回
...
func officialCodex0145ResolveEndpointTLSProfileForURL(
	version string,
	endpointID codex0145EndpointID,
	target *url.URL,
) (*tlsfingerprint.Profile, error) {
	...
	if !endpoint.HostFromResponse &&
		!officialCodex0145EscapedPathMatchesTemplate(endpoint.Path, target.EscapedPath()) {
		return nil, fmt.Errorf("Codex 端点 %s 的 URL path %q 不匹配画像 %q", endpoint.ID, target.EscapedPath(), endpoint.Path)
	}

	profile, err := officialCodex0145ResolveTLSProfile(version, endpoint.TransportID)
	if err != nil {
		return nil, err
	}
	replaced := false
...
func officialCodex0145EscapedPathMatchesTemplate(template, escapedPath string) bool {
...
// officialCodex0145CompileH1Rules 为同一传输编译完整端点矩阵，不产生空路径兜底。
...
func officialCodex0145CompileH1Rules(
	...
	for _, endpoint := range versionProfile.Endpoints {
		...
		prefix := officialCodex0145LowerHeaderNames(transport.WebSocket.FixedHandshakePrefix)
		...
		initialOrder, appendHeaders, err := officialCodex0145CompileWSHeaderConstruction(
			endpoint,
			desired,
			prefix,
		)
...
// officialCodex0145CompileWSHeaderConstruction 把端点画像中的构造阶段数据编译为
// wire 执行规则，并验证“初始 HeaderMap + 删除后追加”恰好覆盖端点 header 闭集。
// 这使新增版本必须显式给出源码/抓包可追溯的构造序，不能退回完整样本反推。
func officialCodex0145CompileWSHeaderConstruction(
	endpoint officialCodexEndpointProfile,
	desired []string,
	prefix []string,
) ([]string, []string, error) {
	initialOrder := officialCodex0145LowerHeaderNames(endpoint.HeaderMapInsertionOrder)
	appendHeaders := officialCodex0145LowerHeaderNames(endpoint.PostRemoveHeaders)
...
func officialCodex0145LowerHeaderNames(names []string) []string {
...
// officialCodex0145BuildEndpointURL 根据端点画像生成完整 URL。固定 query 只能来自
// 画像，动态 query 必须逐项提供；任何画像外字段都会失败，禁止把入站 query 整体
// 透传到官方端点。
func officialCodex0145BuildEndpointURL(
	version string,
	endpointID codex0145EndpointID,
	...
	if endpoint.HostFromResponse {
		return officialCodex0145ValidateReturnedURL(endpoint, input)
	}
	if strings.TrimSpace(input.ReturnedURL) != "" {
		return nil, fmt.Errorf("Codex 端点 %s 不接受服务端返回 URL", endpoint.ID)
	}

	expandedPath, rawPath, usedPathValues, err := officialCodex0145ExpandEndpointPath(endpoint.Path, input.PathValues)
	if err != nil {
...
func officialCodex0145ExpandEndpointPath(
	template string,
...
func officialCodex0145ValidateReturnedURL(
	endpoint officialCodexEndpointProfile,
	input officialCodex0145EndpointURLInput,
) (*url.URL, error) {
...
func officialCodex0145HostMatches(pattern, host string) bool {
...
// officialCodex0145ApplyHeaderContract 把 http.Header 收敛为端点画像的闭集，并
...
func officialCodex0145ApplyHeaderContract(
	version string,
	endpointID codex0145EndpointID,
	headers http.Header,
	...
	if headers == nil {
		return nil, errors.New("Codex 0.145.0 header 契约输入为空")
	}
	endpoint, err := resolveCodex0145Endpoint(version, endpointID)
	if err != nil {
		return nil, err
	...
	fields := make([]officialCodex0145HeaderField, 0, len(orderedSlots))
	for index, slot := range orderedSlots {
		name := strings.ToLower(slot.Name)
		if !active[index] {
			delete(values, name)
		...
		if officialCodex0145TransportGeneratesHeader(name) {
		...
		values[name] = value
		fields = append(fields, officialCodex0145HeaderField{
...
func officialCodex0145TransportGeneratesHeader(name string) bool {
...
// officialCodex0145ValidateAndOrderJSONBody 对 JSON 类端点执行顶层闭集校验，并按
// 画像字段顺序重新编码。它不会通过 map 再 Marshal，因此字段顺序不受 Go map
// 迭代影响；每个字段值只做 JSON 压缩，不改变数字或字符串语义。
func officialCodex0145ValidateAndOrderJSONBody(
	version string,
	endpointID codex0145EndpointID,
	body []byte,
	conditions map[string]bool,
	...
	pairs, err := officialCodex0145DecodeJSONObject(body)
	if err != nil {
	...
	for _, field := range endpoint.Body.Fields {
		...
		if present && officialCodex0145ShouldOmitJSONField(field.OmitWhen, value) {
	...
	if err := officialCodex0145ValidateJSONDiscriminator(endpoint, values); err != nil {
		return nil, err
	}
...
// officialCodex0145ValidateAndOrderFormBody 对 form_urlencoded 端点执行与 JSON
// Finalizer 相同的闭集和稳定顺序校验。OAuth refresh 的字段顺序来自版本画像，
// 不能依赖调用方 map 或通用表单库的排序实现。
func officialCodex0145ValidateAndOrderFormBody(
	version string,
	endpointID codex0145EndpointID,
	values url.Values,
) ([]byte, error) {
...
type officialCodex0145JSONPair struct {
	name  string
	value json.RawMessage
}

func officialCodex0145DecodeJSONObject(body []byte) ([]officialCodex0145JSONPair, error) {
	...
	pairs := make([]officialCodex0145JSONPair, 0)
	for decoder.More() {
		...
		pairs = append(pairs, officialCodex0145JSONPair{name: name, value: value})
	}
...
func officialCodex0145ShouldOmitJSONField(omitWhen string, value json.RawMessage) bool {
...
func officialCodex0145ValidateJSONDiscriminator(
	endpoint officialCodexEndpointProfile,
