...
const (
	codex01491R9ContaminationRecoveryServicePath = "docs/egress/maintenance/codex-0.149.1-r9-contamination-recovery-transition.json"
	codex01491ModelCatalogH1SuccessorServicePath = "docs/egress/maintenance/codex-0.149.1-model-catalog-h1-successor-transition.json"
	codex01491R11AHarnessRepairServicePath       = "docs/egress/maintenance/codex-0.149.1-r11a-harness-repair-transition.json"
	codex01491R11BRelayCompletionServicePath     = "docs/egress/maintenance/codex-0.149.1-r11b-relay-completion-transition.json"
)

type codex01491R9ContaminationRecoveryServiceReceipt struct {
...
type codex01491R11AHarnessRepairServiceReceipt struct {
	SchemaVersion         string `json:"schema_version"`
	IssuedAtUTC           string `json:"issued_at_utc"`
	BaseCommit            string `json:"base_commit"`
	Scope                 string `json:"scope"`
...
type codex01491R11BRelayCompletionServiceReceipt struct {
	SchemaVersion         string `json:"schema_version"`
	IssuedAtUTC           string `json:"issued_at_utc"`
	BaseCommit            string `json:"base_commit"`
	Scope                 string `json:"scope"`
...
func codex01491R9ContaminationRecoveryExpectedServicePaths() []string {
	return []string{
...
func codex01491R11AHarnessRepairExpectedServicePaths() []string {
	return []string{
		"backend/internal/officialegress/codex_01491_r9_contamination_recovery_transition_test.go",
		"backend/internal/service/codex_01491_r9_contamination_recovery_transition_test.go",
...
func codex01491R11BRelayCompletionExpectedServicePaths() []string {
	return []string{
		"backend/internal/officialegress/codex_01491_r9_contamination_recovery_transition_test.go",
		"backend/internal/service/codex_01491_r9_contamination_recovery_transition_test.go",
...
func loadCodex01491ModelCatalogH1SuccessorServiceTransitions() (
	[]codex01491CandidateSourceServiceEntry,
	error,
) {
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491ModelCatalogH1SuccessorServicePath),
	))
	if err != nil {
		return nil, err
	}
	if upstreamMergeFrameworkServiceDigest(raw) != "e97b35438e82f4ee7adb905eadbb5483c035cd387b144e00d39c63321988bba6" {
		return nil, errors.New("Codex 0.149.1 service 模型目录 H1 后继 transition 文件摘要非法")
	}
	var receipt struct {
		SchemaVersion  string                                  `json:"schema_version"`
		Scope          string                                  `json:"scope"`
	...
	if receipt.SchemaVersion != "official-client-codex-0.149.1-model-catalog-h1-successor-transition/v1" ||
		...
		return nil, errors.New("Codex 0.149.1 service 模型目录 H1 后继 transition 身份非法")
	}
...
func loadCodex01491R11BRelayCompletionServiceTransition() (
	codex01491R11BRelayCompletionServiceReceipt,
	error,
) {
	var receipt codex01491R11BRelayCompletionServiceReceipt
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
	...
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return receipt, errors.New("Codex 0.149.1 service r11b relay transition 尾部存在额外 JSON")
	}
	if err := codex01491VerifyCandidateGateServiceIdentity(raw, receipt.IdentitySHA256); err != nil {
		return receipt, err
	}
	if err := validateCodex01491R11BRelayCompletionServiceTransition(receipt); err != nil {
		return receipt, err
	}
	successor, err := loadCodex01491R11CModelsSyncServiceTransition()
	if err != nil {
		return receipt, err
	}
...
func validateCodex01491R11BRelayCompletionServiceTransition(
	receipt codex01491R11BRelayCompletionServiceReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r11b-relay-completion-transition/v1" ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 顶层事实非法")
	}
	if _, err := time.Parse(time.RFC3339, receipt.IssuedAtUTC); err != nil {
		return errors.New("Codex 0.149.1 service r11b relay transition 时间非法")
	}
	predecessorRaw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491R11AHarnessRepairServicePath),
	))
	if err != nil {
		return err
	}
	...
	if receipt.PredecessorTransition.Path != codex01491R11AHarnessRepairServicePath ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 前序绑定非法")
	}
	if receipt.FailedOfficialAttempt.CampaignID != "codex-01491-r11a" ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 失败 Attempt 事实非法")
	}
	if receipt.Boundaries.APIKeyRef != "#4" || receipt.Boundaries.CaptureAccountRef != "#21" ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 账号或网络边界非法")
	}
	if receipt.FailureFacts.RelayAttemptCount != 3 || receipt.FailureFacts.ModelsHTTPStatus != 200 ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 失败根因事实非法")
	}
	if !receipt.RepairFacts.WaitInsideAppServerLifetime ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 修复事实非法")
	}
	if !receipt.Verification.FailedAttemptRestorationVerified ||
		...
		return errors.New("Codex 0.149.1 service r11b relay transition 验证事实非法")
	}

	expectedPaths := codex01491R11BRelayCompletionExpectedServicePaths()
	...
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 service r11b relay transition 路径数量非法")
	}
	for index, entry := range receipt.Transitions {
		...
		if entry.Path != expectedPath || entry.Change != expectedChange ||
			...
			return errors.New("Codex 0.149.1 service r11b relay transition 条目非法：" + expectedPath)
		}
		...
		if readErr != nil || (currentDigest != entry.ToSHA256 &&
			!codex01491R11CModelsSyncSupersedesService(
				entry.Path,
				entry.ToSHA256,
				currentDigest,
			)) {
			return errors.New("Codex 0.149.1 service r11b relay transition 当前摘要不一致：" + entry.Path)
		}
	}
	return nil
}

func codex01491R11BRelayCompletionSupersedesService(
	path string,
	priorDigest string,
...
func loadCodex01491R11AHarnessRepairServiceTransition() (
	codex01491R11AHarnessRepairServiceReceipt,
	error,
) {
	var receipt codex01491R11AHarnessRepairServiceReceipt
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491R11AHarnessRepairServicePath),
	))
	...
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return receipt, errors.New("Codex 0.149.1 service r11a 脚手架 transition 尾部存在额外 JSON")
	}
	if err := codex01491VerifyCandidateGateServiceIdentity(raw, receipt.IdentitySHA256); err != nil {
		return receipt, err
	}
	if err := validateCodex01491R11AHarnessRepairServiceTransition(receipt); err != nil {
		return receipt, err
	}
	successor, err := loadCodex01491R11BRelayCompletionServiceTransition()
	if err != nil {
		return receipt, err
	}
	receipt.Transitions = append(receipt.Transitions, successor.Transitions...)
...
func validateCodex01491R11AHarnessRepairServiceTransition(
	receipt codex01491R11AHarnessRepairServiceReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r11a-harness-repair-transition/v1" ||
		...
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 顶层事实非法")
	}
	if _, err := time.Parse(time.RFC3339, receipt.IssuedAtUTC); err != nil {
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 时间非法")
	}
	predecessorRaw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491R9ContaminationRecoveryServicePath),
	))
	if err != nil {
		return err
	}
	...
	if receipt.PredecessorTransition.Path != codex01491R9ContaminationRecoveryServicePath ||
		receipt.PredecessorTransition.FileSHA256 != upstreamMergeFrameworkServiceDigest(predecessorRaw) ||
		receipt.PredecessorTransition.IdentitySHA256 != predecessor.IdentitySHA256 {
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 前序绑定非法")
	}
	if receipt.FailedCandidateAttempt.CampaignID != "codex-01491-r10a" ||
		...
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 失败 Attempt 事实非法")
	}
	if receipt.Boundaries.APIKeyRef != "#4" || receipt.Boundaries.CaptureAccountRef != "#21" ||
		receipt.Boundaries.CaptureAccount20Allowed || receipt.Boundaries.CaptureAccount20Used ||
		receipt.Boundaries.ServicePrivateIP != "172.25.0.3" ||
		...
		receipt.Boundaries.ProxyOrComposeNetworkChanged || receipt.Boundaries.ProductionSelectorChanged ||
		!receipt.Boundaries.HistoricalEvidencePreservedReadOnly || receipt.Boundaries.VircsAccessed {
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 账号或网络边界非法")
	}
	if !receipt.RepairFacts.PublishedPortAcceptsNonLoopbackBinding ||
		...
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 修复事实非法")
	}
	if receipt.PreflightEvidence.H1RunID != "codex-01491-r10b-preflight-h1-final" ||
		...
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 预检事实非法")
	}
	if !receipt.Verification.BashSyntaxPassed || !receipt.Verification.TargetedPythonTestsPassed ||
		...
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 验证事实非法")
	}

	expectedPaths := codex01491R11AHarnessRepairExpectedServicePaths()
	expectedPredecessors := map[string][]string{
	...
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 service r11a 脚手架 transition 路径数量非法")
	}
	for index, entry := range receipt.Transitions {
		expectedPath := expectedPaths[index]
		predecessors := expectedPredecessors[expectedPath]
		...
		if entry.Path != expectedPath || entry.Change != expectedChange ||
			!slices.Equal(entry.PredecessorSHA256s, predecessors) || len(entry.ToSHA256) != 64 ||
			strings.TrimSpace(entry.Reason) == "" {
			return errors.New("Codex 0.149.1 service r11a 脚手架 transition 条目非法：" + expectedPath)
		}
		current, readErr := os.ReadFile(filepath.Join("../../..", filepath.FromSlash(entry.Path)))
		currentDigest := upstreamMergeFrameworkServiceDigest(current)
		if readErr != nil || (currentDigest != entry.ToSHA256 &&
			!codex01491R11BRelayCompletionSupersedesService(
				entry.Path,
				entry.ToSHA256,
				currentDigest,
			)) {
			return errors.New("Codex 0.149.1 service r11a 脚手架 transition 当前摘要不一致：" + entry.Path)
		}
	}
	return nil
}

func codex01491R11AHarnessRepairSupersedesService(
	path string,
	priorDigest string,
	currentDigest string,
) bool {
...
func loadCodex01491R9ContaminationRecoveryServiceTransition() (
	codex01491R9ContaminationRecoveryServiceReceipt,
	error,
) {
	var receipt codex01491R9ContaminationRecoveryServiceReceipt
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491R9ContaminationRecoveryServicePath),
	))
	...
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return receipt, errors.New("Codex 0.149.1 service r9 污染恢复 transition 尾部存在额外 JSON")
	}
	if err := codex01491VerifyCandidateGateServiceIdentity(raw, receipt.IdentitySHA256); err != nil {
		return receipt, err
	}
	if err := validateCodex01491R9ContaminationRecoveryServiceTransition(receipt); err != nil {
		return receipt, err
	}
	successor, err := loadCodex01491R11AHarnessRepairServiceTransition()
	if err != nil {
		return receipt, err
	}
	receipt.Transitions = append(receipt.Transitions, successor.Transitions...)
...
func validateCodex01491R9ContaminationRecoveryServiceTransition(
	receipt codex01491R9ContaminationRecoveryServiceReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r9-contamination-recovery-transition/v1" ||
		...
		strings.TrimSpace(receipt.IssuedAtUTC) == "" {
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition 顶层事实非法")
	}
	predecessorRaw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491ModelCatalogH1SuccessorServicePath),
	...
	if receipt.PredecessorTransition.Path != codex01491ModelCatalogH1SuccessorServicePath ||
		receipt.PredecessorTransition.FileSHA256 != upstreamMergeFrameworkServiceDigest(predecessorRaw) ||
		receipt.PredecessorTransition.IdentitySHA256 != predecessor.IdentitySHA256 {
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition 前序绑定非法")
	}
	if receipt.FailedCandidateAttempt.CampaignID != "codex-01491-r9" ||
		...
		!receipt.FailedCandidateAttempt.ReuseForbidden {
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition Attempt 事实非法")
	}
	if receipt.Boundaries.APIKeyRef != "#4" || receipt.Boundaries.CaptureAccountRef != "#21" ||
		receipt.Boundaries.CaptureAccount20Allowed || receipt.Boundaries.CaptureAccount20Used ||
		...
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition 账号或生产边界非法")
	}
	if receipt.RepairFacts.CaptureCLICandidateNetworkRequired != "sub2apiplus_sub2api-network" ||
		...
		!receipt.Verification.BashSyntaxPassed || !receipt.Verification.CaptureToolTestsPassed ||
		...
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition 修复或验证事实非法")
	}
	if !receipt.Safety.ActiveRemained01470 || receipt.Safety.ARM64DeploymentPerformed ||
		...
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition 安全边界非法")
	}

	expectedPaths := codex01491R9ContaminationRecoveryExpectedServicePaths()
	expectedPredecessors := map[string][]string{
	...
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 service r9 污染恢复 transition 路径数量非法")
	}
	for index, entry := range receipt.Transitions {
		expectedPath := expectedPaths[index]
		predecessors := expectedPredecessors[expectedPath]
		...
		if entry.Path != expectedPath || entry.Change != expectedChange ||
			!slices.Equal(entry.PredecessorSHA256s, predecessors) || len(entry.ToSHA256) != 64 ||
			strings.TrimSpace(entry.Reason) == "" {
			return errors.New("Codex 0.149.1 service r9 污染恢复 transition 条目非法：" + expectedPath)
		}
		current, readErr := os.ReadFile(filepath.Join("../../..", filepath.FromSlash(entry.Path)))
		currentDigest := upstreamMergeFrameworkServiceDigest(current)
		if readErr != nil || (currentDigest != entry.ToSHA256 &&
			!codex01491R11AHarnessRepairSupersedesService(
				entry.Path,
				entry.ToSHA256,
				currentDigest,
			)) {
			return errors.New("Codex 0.149.1 service r9 污染恢复 transition 当前摘要不一致：" + entry.Path)
		}
	}
	return nil
}

func codex01491R9ContaminationRecoverySupersedesService(
	path string,
	priorDigest string,
	currentDigest string,
) bool {
...
func TestCodex01491R9ContaminationRecoveryServiceTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R9ContaminationRecoveryServiceTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateCodex01491R9ContaminationRecoveryServiceTransition(mutated); err == nil {
		t.Fatal("Codex 0.149.1 service r9 污染恢复 transition 接受了账号 #20")
	}
}

func TestCodex01491R11AHarnessRepairServiceTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R11AHarnessRepairServiceTransition()
	if err != nil {
		t.Fatal(err)
	}
	mutated := receipt
	mutated.Boundaries.CaptureAccountRef = "#20"
	if err := validateCodex01491R11AHarnessRepairServiceTransition(mutated); err == nil {
		t.Fatal("Codex 0.149.1 service r11a 脚手架 transition 接受了账号 #20")
	}
	mutated = receipt
	mutated.Boundaries.RouteNATIPTablesChanged = true
	if err := validateCodex01491R11AHarnessRepairServiceTransition(mutated); err == nil {
		t.Fatal("Codex 0.149.1 service r11a 脚手架 transition 接受了路由、NAT 或 iptables 变更")
	}
}

func TestCodex01491R11BRelayCompletionServiceTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R11BRelayCompletionServiceTransition()
	if err != nil {
		t.Fatal(err)
	}
	mutated := receipt
	mutated.Boundaries.CaptureAccountRef = "#20"
	if err := validateCodex01491R11BRelayCompletionServiceTransition(mutated); err == nil {
		t.Fatal("Codex 0.149.1 service r11b relay transition 接受了账号 #20")
	}
	mutated = receipt
	mutated.RepairFacts.NetworkConfigurationChanged = true
	if err := validateCodex01491R11BRelayCompletionServiceTransition(mutated); err == nil {
		t.Fatal("Codex 0.149.1 service r11b relay transition 接受了网络配置变更")
	}
}
