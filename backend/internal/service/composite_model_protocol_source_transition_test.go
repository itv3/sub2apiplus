...
func loadCompositeModelProtocolSourceTransition() (
	...
	if upstreamMergeFrameworkServiceDigest(raw) != compositeModelProtocolSourceTransitionSHA {
...
func validateCompositeModelProtocolSourceTransition(
	...
	if receipt.Predecessor.Kind != "upstream_release_ci_repair" ||
		receipt.Predecessor.Path != upstreamV0179ReleaseCIRepairTransitionServicePath ||
		receipt.Predecessor.SHA256 != upstreamV0179ReleaseCIRepairTransitionServiceSHA {
...
// compositeModelProtocolSourceTransitionSupersedesService 只接受本次模型目录
// 协议隔离收据中的精确 path/from/to，不扩大到 OAuth 出站画像或其他网关入口。
func compositeModelProtocolSourceTransitionSupersedesService(
	path string,
	priorDigest string,
