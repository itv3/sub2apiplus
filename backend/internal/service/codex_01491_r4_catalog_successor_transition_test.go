...
const codex01491R4CatalogSuccessorServicePath = "docs/egress/maintenance/codex-0.149.1-r4-catalog-successor-transition.json"

type codex01491R4CatalogBindingService struct {
	Path              string `json:"path"`
	PredecessorSHA256 string `json:"predecessor_sha256,omitempty"`
	SHA256            string `json:"sha256"`
}

type codex01491R4CatalogSuccessorServiceReceipt struct {
	...
	StagedCatalog struct {
		...
		ReleaseCatalog            codex01491R4CatalogBindingService `json:"release_catalog"`
		ReleaseGraph              codex01491R4CatalogBindingService `json:"release_graph"`
		ContractReleaseGraph      codex01491R4CatalogBindingService `json:"contract_release_graph"`
...
var (
	codex01491R4CatalogSuccessorServiceOnce    sync.Once
	codex01491R4CatalogSuccessorServiceCached  codex01491R4CatalogSuccessorServiceReceipt
	codex01491R4CatalogSuccessorServiceLoadErr error
)

func codex01491R4CatalogSuccessorExpectedServicePaths() []string {
	return []string{
...
func loadCodex01491R4CatalogSuccessorServiceTransition() (
	codex01491R4CatalogSuccessorServiceReceipt,
	error,
) {
	codex01491R4CatalogSuccessorServiceOnce.Do(func() {
		raw, err := os.ReadFile(filepath.Join(
			"../../..",
			filepath.FromSlash(codex01491R4CatalogSuccessorServicePath),
		))
		if err != nil {
			codex01491R4CatalogSuccessorServiceLoadErr = err
			return
		}
		decoder := json.NewDecoder(bytes.NewReader(raw))
		decoder.DisallowUnknownFields()
		if err := decoder.Decode(&codex01491R4CatalogSuccessorServiceCached); err != nil {
			codex01491R4CatalogSuccessorServiceLoadErr = err
			return
		}
		if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
			codex01491R4CatalogSuccessorServiceLoadErr = errors.New("Codex 0.149.1 service r4 Catalog 后继 transition 尾部存在额外 JSON")
			return
		}
		if err := codex01491VerifyCandidateGateServiceIdentity(
			raw,
			codex01491R4CatalogSuccessorServiceCached.IdentitySHA256,
		); err != nil {
			codex01491R4CatalogSuccessorServiceLoadErr = err
			return
		}
		codex01491R4CatalogSuccessorServiceLoadErr = validateCodex01491R4CatalogSuccessorServiceTransition(
			codex01491R4CatalogSuccessorServiceCached,
		)
	})
	return codex01491R4CatalogSuccessorServiceCached, codex01491R4CatalogSuccessorServiceLoadErr
}

func validateCodex01491R4CatalogSuccessorServiceTransition(
	receipt codex01491R4CatalogSuccessorServiceReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r4-catalog-successor-transition/v1" ||
		...
		receipt.Result != "r4_candidate_catalog_successor_frozen" || len(receipt.Verification) == 0 {
		return errors.New("Codex 0.149.1 service r4 Catalog 后继 transition 顶层事实非法")
	}
	if receipt.Campaign.ID != "codex-0_149_1-formal-production-replacement-20260826T140949Z-2c1ab3b9e-r4" ||
		...
		receipt.Campaign.Purpose != "production_replacement" || receipt.Campaign.TargetVersion != "0.149.1" ||
		...
		return errors.New("Codex 0.149.1 service r4 Catalog 后继 transition Campaign 身份非法")
	}
	if receipt.StagedCatalog.ActiveVersion != "0.147.0" || receipt.StagedCatalog.PreviousVersion != "0.149.1" ||
		!receipt.StagedCatalog.ActiveUnchanged || receipt.StagedCatalog.ProductionSelectorChanged ||
		receipt.StagedCatalog.ReleaseCatalog.SHA256 != "c26b274a5942eee249ae4755aa60c399745f03212077e798fb5d582f0ce6c81e" ||
		receipt.StagedCatalog.ReleaseGraph.SHA256 != "9824eee0200ea1be1136a1a87ea5accc9a8e5a7c48855b4f0c06587eeab17ca4" ||
		receipt.StagedCatalog.ContractReleaseGraph.SHA256 != "9824eee0200ea1be1136a1a87ea5accc9a8e5a7c48855b4f0c06587eeab17ca4" ||
		receipt.StagedCatalog.InventorySHA256 != "f00545f1296175c1a60f51ec770bf050832dbf80bbc61569fbfe9ec78759111b" {
		return errors.New("Codex 0.149.1 service r4 Catalog 后继 transition 暂存 Catalog 事实非法")
	}
	if !receipt.Safety.ActiveRemained01470 || !receipt.Safety.PreviousStaged01491 ||
		...
		receipt.Safety.HistoricalContentAddressedArtifactsOverwrote || receipt.Safety.HistoricalReceiptsModified ||
		receipt.Safety.HistoricalTransitionsModified || receipt.Safety.DeploymentPerformed ||
		receipt.Safety.LiveRequestSent || receipt.Safety.ARM64AccessedForThisTransition ||
		receipt.Safety.VircsAccessed || receipt.Safety.CaptureAccount20Used ||
		!receipt.Safety.CaptureAccount21Used {
		return errors.New("Codex 0.149.1 service r4 Catalog 后继 transition 安全边界非法")
	}
	expectedPaths := codex01491R4CatalogSuccessorExpectedServicePaths()
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 service r4 Catalog 后继 transition 路径数量非法")
	}
	...
	if upstreamMergeFrameworkServiceDigest(pathRaw) != receipt.PathSetSHA256 {
		return errors.New("Codex 0.149.1 service r4 Catalog 后继 transition 路径摘要不一致")
	}
	return nil
}

func codex01491R4CatalogSuccessorSupersedesService(
	path string,
	priorDigest string,
	...
	r13, err := loadCodex01491R13CandidateCoordinateServiceTransition()
	if err != nil {
		return false
	}
...
func TestCodex01491R4CatalogSuccessorServiceTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R4CatalogSuccessorServiceTransition()
	if err != nil {
		t.Fatal(err)
	}
	entry := receipt.Transitions[0]
	if !codex01491R4CatalogSuccessorSupersedesService(
		entry.Path,
		...
		t.Fatal("Codex 0.149.1 service r4 Catalog 后继 transition 精确三元组判据非法")
	}
}
