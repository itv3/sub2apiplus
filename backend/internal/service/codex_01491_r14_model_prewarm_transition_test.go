...
const codex01491R14ModelPrewarmServicePath = "docs/egress/maintenance/codex-0.149.1-r14-model-prewarm-transition.json"

type codex01491R14ModelPrewarmServiceReceipt struct {
...
var (
	codex01491R14ModelPrewarmServiceOnce   sync.Once
	codex01491R14ModelPrewarmServiceCached codex01491R14ModelPrewarmServiceReceipt
	codex01491R14ModelPrewarmServiceError  error
)

func codex01491R14ModelPrewarmExpectedServicePaths() []string {
	return []string{
...
func codex01491R14ModelPrewarmServiceSectionDigest(raw json.RawMessage) (string, error) {
	var value any
...
func loadCodex01491R14ModelPrewarmServiceTransition() (
	codex01491R14ModelPrewarmServiceReceipt,
	error,
) {
	codex01491R14ModelPrewarmServiceOnce.Do(func() {
		codex01491R14ModelPrewarmServiceCached, codex01491R14ModelPrewarmServiceError =
			loadCodex01491R14ModelPrewarmServiceTransitionUncached()
	})
	return codex01491R14ModelPrewarmServiceCached, codex01491R14ModelPrewarmServiceError
}

func loadCodex01491R14ModelPrewarmServiceTransitionUncached() (
	codex01491R14ModelPrewarmServiceReceipt,
	error,
) {
	var receipt codex01491R14ModelPrewarmServiceReceipt
	raw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491R14ModelPrewarmServicePath),
	))
	if err != nil {
		return receipt, err
	}
	...
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return receipt, errors.New("Codex 0.149.1 service r14 模型预热 transition 尾部存在额外 JSON")
	}
	if err := codex01491VerifyCandidateGateServiceIdentity(raw, receipt.IdentitySHA); err != nil {
		return receipt, err
	}
	return receipt, validateCodex01491R14ModelPrewarmServiceTransition(receipt)
}

func validateCodex01491R14ModelPrewarmServiceTransition(
	receipt codex01491R14ModelPrewarmServiceReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r14-model-prewarm-transition/v1" ||
		...
		return errors.New("Codex 0.149.1 service r14 模型预热 transition 顶层事实非法")
	}
	if _, err := time.Parse(time.RFC3339, receipt.IssuedAtUTC); err != nil {
		return errors.New("Codex 0.149.1 service r14 模型预热 transition 时间非法")
	}
	predecessorRaw, err := os.ReadFile(filepath.Join(
		"../../..",
		filepath.FromSlash(codex01491R13CandidateCoordinateServicePath),
	))
	if err != nil {
		return err
	}
	...
	if receipt.PredecessorTransition.Path != codex01491R13CandidateCoordinateServicePath ||
		...
		return errors.New("Codex 0.149.1 service r14 模型预热 transition 前序绑定非法")
	}
	...
	for name, raw := range sections {
		digest, digestErr := codex01491R14ModelPrewarmServiceSectionDigest(raw)
		if digestErr != nil || digest != expectedSections[name] {
			return errors.New("Codex 0.149.1 service r14 模型预热 transition 事实非法：" + name)
		}
	}

	expectedPaths := codex01491R14ModelPrewarmExpectedServicePaths()
	...
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 service r14 模型预热 transition 路径数量非法")
	}
	...
	if upstreamMergeFrameworkServiceDigest(pathRaw) != receipt.PathSetSHA256 {
		return errors.New("Codex 0.149.1 service r14 模型预热 transition 路径摘要不一致")
	}
	return nil
}

func codex01491R14ModelPrewarmSupersedesService(
	path string,
	priorDigest string,
...
func TestCodex01491R14ModelPrewarmServiceTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R14ModelPrewarmServiceTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateCodex01491R14ModelPrewarmServiceTransition(receipt); err == nil {
		t.Fatal("Codex 0.149.1 service r14 模型预热 transition 接受了账号 #20")
	}
}
