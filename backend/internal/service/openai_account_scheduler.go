...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			...
			if !groupCompatible ||
				!compatible || !s.isAccountTransportCompatible(ctx, selection.Account, req.RequiredTransport) {
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if account == nil || !s.service.openAIAccountMatchesSchedulingGroup(account, req.GroupID) || !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(ctx, account, req.RequiredTransport) {
		clearBinding()
		return nil, false, nil
	}
