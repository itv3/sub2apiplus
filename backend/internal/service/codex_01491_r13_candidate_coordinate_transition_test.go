...
const codex01491R13CandidateCoordinateServicePath = "docs/egress/maintenance/codex-0.149.1-r13-candidate-coordinate-transition.json"

type codex01491R13CandidateCoordinateServiceReceipt struct {
...
var (
	codex01491R13CandidateCoordinateServiceOnce   sync.Once
	codex01491R13CandidateCoordinateServiceCached codex01491R13CandidateCoordinateServiceReceipt
	codex01491R13CandidateCoordinateServiceError  error
)

func codex01491R13CandidateCoordinateExpectedServicePaths() []string {
	return []string{
...
func codex01491R13CandidateCoordinateServiceSectionDigest(raw json.RawMessage) (string, error) {
	var value any
...
func loadCodex01491R13CandidateCoordinateServiceTransition() (
	codex01491R13CandidateCoordinateServiceReceipt,
	error,
) {
	codex01491R13CandidateCoordinateServiceOnce.Do(func() {
		codex01491R13CandidateCoordinateServiceCached, codex01491R13CandidateCoordinateServiceError =
			loadCodex01491R13CandidateCoordinateServiceTransitionUncached()
	})
	return codex01491R13CandidateCoordinateServiceCached, codex01491R13CandidateCoordinateServiceError
}

func loadCodex01491R13CandidateCoordinateServiceTransitionUncached() (
	codex01491R13CandidateCoordinateServiceReceipt,
	error,
) {
	var receipt codex01491R13CandidateCoordinateServiceReceipt
	...
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return receipt, errors.New("Codex 0.149.1 service r13 候选坐标 transition 尾部存在额外 JSON")
	}
...
func validateCodex01491R13CandidateCoordinateServiceTransition(
	receipt codex01491R13CandidateCoordinateServiceReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r13-candidate-coordinate-transition/v1" ||
		...
		return errors.New("Codex 0.149.1 service r13 候选坐标 transition 顶层事实非法")
	}
	if _, err := time.Parse(time.RFC3339, receipt.IssuedAtUTC); err != nil {
		return errors.New("Codex 0.149.1 service r13 候选坐标 transition 时间非法")
	}
	...
	if receipt.PredecessorTransition.Path != codex01491R12EPreconnectServicePath ||
		...
		return errors.New("Codex 0.149.1 service r13 候选坐标 transition 前序绑定非法")
	}
	...
	for name, raw := range sections {
		digest, digestErr := codex01491R13CandidateCoordinateServiceSectionDigest(raw)
		if digestErr != nil || digest != expectedSections[name] {
			return errors.New("Codex 0.149.1 service r13 候选坐标 transition 事实非法：" + name)
		}
	}

	expectedPaths := codex01491R13CandidateCoordinateExpectedServicePaths()
	...
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 service r13 候选坐标 transition 路径数量非法")
	}
	...
	if upstreamMergeFrameworkServiceDigest(pathRaw) != receipt.PathSetSHA256 {
		return errors.New("Codex 0.149.1 service r13 候选坐标 transition 路径摘要不一致")
	}
	return nil
}

func codex01491R13CandidateCoordinateSupersedesService(
	path string,
	priorDigest string,
...
func TestCodex01491R13CandidateCoordinateServiceTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R13CandidateCoordinateServiceTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := validateCodex01491R13CandidateCoordinateServiceTransition(receipt); err == nil {
		t.Fatal("Codex 0.149.1 service r13 候选坐标 transition 接受了账号 #20")
	}
}
