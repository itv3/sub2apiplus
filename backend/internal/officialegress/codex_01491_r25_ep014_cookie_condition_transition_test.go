...
const codex01491R25EP014CookieConditionPath = "docs/egress/maintenance/codex-0.149.1-r25-ep014-cookie-condition-transition.json"

type codex01491R25EP014CookieConditionReceipt struct {
...
var (
	codex01491R25EP014CookieConditionOnce   sync.Once
	codex01491R25EP014CookieConditionCached codex01491R25EP014CookieConditionReceipt
	codex01491R25EP014CookieConditionError  error
)

func codex01491R25EP014CookieConditionExpectedPaths() []string {
	return []string{
...
func loadCodex01491R25EP014CookieConditionTransition() (
	codex01491R25EP014CookieConditionReceipt,
	error,
) {
	codex01491R25EP014CookieConditionOnce.Do(func() {
		raw, err := codex01491RepoFile(codex01491R25EP014CookieConditionPath)
		if err != nil {
			codex01491R25EP014CookieConditionError = err
			return
		}
		decoder := json.NewDecoder(bytes.NewReader(raw))
		decoder.DisallowUnknownFields()
		if err := decoder.Decode(&codex01491R25EP014CookieConditionCached); err != nil {
			codex01491R25EP014CookieConditionError = err
			return
		}
		if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
			codex01491R25EP014CookieConditionError = errors.New("Codex 0.149.1 r25 EP-014 transition 尾部存在额外 JSON")
			return
		}
		if err := codex01491VerifyIdentity(
			raw,
			codex01491R25EP014CookieConditionCached.IdentitySHA256,
		); err != nil {
			codex01491R25EP014CookieConditionError = err
			return
		}
		codex01491R25EP014CookieConditionError =
			validateCodex01491R25EP014CookieConditionTransition(
				codex01491R25EP014CookieConditionCached,
			)
	})
	return codex01491R25EP014CookieConditionCached,
		codex01491R25EP014CookieConditionError
}

func validateCodex01491R25EP014CookieConditionTransition(
	receipt codex01491R25EP014CookieConditionReceipt,
) error {
	if receipt.SchemaVersion != "official-client-codex-0.149.1-r25-ep014-cookie-condition-transition/v1" ||
		...
		return errors.New("Codex 0.149.1 r25 EP-014 transition 顶层事实非法")
	}
	if _, err := time.Parse(time.RFC3339, receipt.IssuedAtUTC); err != nil {
		return errors.New("Codex 0.149.1 r25 EP-014 transition 时间非法")
	}
	predecessorRaw, err := codex01491RepoFile("docs/egress/maintenance/codex-0.149.1-r24-selector-lite-coordinate-transition.json")
	if err != nil {
		return err
	}
	...
	if receipt.Predecessor.Path != "docs/egress/maintenance/codex-0.149.1-r24-selector-lite-coordinate-transition.json" ||
		...
		predecessor.IdentitySHA256 != "3fd50b27cd48f8a9df0b61e697cdf09f36673f765aa9ef81aed611c09fbed96d" {
		return errors.New("Codex 0.149.1 r25 EP-014 transition 前序绑定非法")
	}
	closureRaw, err := codex01491RepoFile(codex01491ServiceSuccessorReplayPath)
	if err != nil {
		return err
	}
	var closure struct {
	...
	if receipt.ToolingClosure.Path != codex01491ServiceSuccessorReplayPath ||
		receipt.ToolingClosure.FileSHA256 != upstreamMergeFrameworkDigest(closureRaw) ||
		receipt.ToolingClosure.IdentitySHA256 != closure.IdentitySHA256 ||
		closure.IdentitySHA256 != "7db2e0f598a4b4b53a7dcb4e7c4c47e05aecb149352793b5516e92d94912a430" {
		return errors.New("Codex 0.149.1 r25 EP-014 transition 工具闭合绑定非法")
	}
	...
	if contract.Reason != "classification_fact_correction" ||
		...
		return errors.New("Codex 0.149.1 r25 EP-014 后继合同非法")
	}
	allowed := []string{
	...
	if fact.RuleID != "SPEC-EP-014" || fact.CheckID != "legacy-default-headers" ||
		...
		return errors.New("Codex 0.149.1 r25 EP-014 条件事实非法")
	}
	...
	if models.MainTrack != "main" || models.MainModel != "gpt-5.5" ||
		...
		return errors.New("Codex 0.149.1 r25 EP-014 模型坐标非法")
	}
	...
	if !verification.OfficialRawHeadersReplayed ||
		...
		return errors.New("Codex 0.149.1 r25 EP-014 验证事实非法")
	}
	...
	if safety.HistoricalArtifactsOverwritten || safety.HistoricalReceiptsModified ||
		...
		return errors.New("Codex 0.149.1 r25 EP-014 安全边界非法")
	}

	expectedPaths := codex01491R25EP014CookieConditionExpectedPaths()
	...
	if len(receipt.Transitions) != len(expectedPaths) {
		return errors.New("Codex 0.149.1 r25 EP-014 transition 路径数量非法")
	}
	...
	for index, entry := range receipt.Transitions {
		...
		if entry.Path != expectedPath || entry.Change != expectedChange ||
			...
			readErr != nil || upstreamMergeFrameworkDigest(current) != entry.ToSHA256 ||
			len(entry.ToSHA256) != 64 || strings.TrimSpace(entry.Reason) == "" ||
			strings.HasPrefix(entry.Path, "docs/egress/maintenance/") {
			return errors.New("Codex 0.149.1 r25 EP-014 transition 条目非法：" + expectedPath)
		}
	...
	if upstreamMergeFrameworkDigest(pathRaw) != receipt.PathSetSHA256 {
		return errors.New("Codex 0.149.1 r25 EP-014 transition 路径摘要非法")
	}
	return nil
}

func codex01491R25EP014CookieConditionSupersedes(
	path string,
	priorDigest string,
	currentDigest string,
) bool {
	receipt, err := loadCodex01491R25EP014CookieConditionTransition()
	if err != nil {
		return false
	}
...
func TestCodex01491R25EP014CookieConditionTransitionIsFrozen(t *testing.T) {
	receipt, err := loadCodex01491R25EP014CookieConditionTransition()
	if err != nil {
		t.Fatal(err)
	}
	...
	if modified.Path == "" || !codex01491R25EP014CookieConditionSupersedes(
		modified.Path,
		...
		t.Fatal("Codex 0.149.1 r25 EP-014 精确三元组判据非法")
	}
	receipt.ModelCoordinates.MainModel = "gpt-5.6-luna"
	receipt.ModelCoordinates.WholeMainSwitchedLuna = true
	if err := validateCodex01491R25EP014CookieConditionTransition(receipt); err == nil {
		t.Fatal("Codex 0.149.1 r25 EP-014 transition 接受了全主线 Luna 扩大")
	}
}
