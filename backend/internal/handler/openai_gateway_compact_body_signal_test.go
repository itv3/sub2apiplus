...
func TestNormalizeOpenAIResponsesCompactRequest_RemoteV2StaysOnResponses(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			nativeV2 := isBareOpenAIResponsesPath(c) && isOpenAIRemoteCompactionV2Request(c, normalized, compactBodySignalTestProfileMode)
...
func TestOpenAIResponsesCompactionRoutingFlags(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                "stream_false_stays_native_v2",
		...
		{
			name:                "stream_absent_stays_native_v2",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			nativeBefore := isBareOpenAIResponsesPath(c) && isOpenAIRemoteCompactionV2Request(c, tt.body, compactBodySignalTestProfileMode)
			...
			nativeAfter := isBareOpenAIResponsesPath(c) && isOpenAIRemoteCompactionV2Request(c, normalized, compactBodySignalTestProfileMode)
