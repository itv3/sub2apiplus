...
func SetupRouter(
	...
	if web.HasEmbeddedFrontend() {
		frontendServer, err := web.NewFrontendServer(settingService) //nolint:staticcheck // 默认 lint 只分析非 embed 桩实现。
		if err != nil {                                              //nolint:staticcheck // 正式 embed 构建仍可能在读取静态资源时返回错误。
