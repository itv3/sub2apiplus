...
Sub2API Plus 是基于 [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) 的个人自用增强版 fork。目标是长期跟随上游升级，同时保留自建镜像、私有部署和 API Key 账号伪装等增强能力。
...
3. API Key 账号伪装通过账号级 `account.Extra` 控制，自定义逻辑尽量放在独立 helper 文件。
...
管理后台已提供账号级聚合配置页：`/admin/settings/api-key-mimic`，侧边栏显示为“API Key 账号伪装”。
