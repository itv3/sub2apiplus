...
OpenAI `/v1/responses/compact` 是特例：compact 请求保持官方 JSON 形态，移除 `stream`、`store`、`prompt_cache_key`、`client_metadata` 等请求级字段，不执行 Codex mimic body 默认值补写，并强制上游 `Accept: application/json`。
...
`backend/internal/service/account_apikey_mimicry.go`、`account_header_override.go`、`gateway_apikey_mimicry.go`、`gateway_tool_rewrite.go`、`openai_apikey_mimicry.go`、`openai_apikey_mimic_profile.go`、`openai_gateway_chat_completions.go`、`openai_gateway_messages.go`、`openai_gateway_service.go`、`openai_codex_transform.go`、`openai_upstream_http.go`、`account_test_service.go`、`account.go`、`backend/internal/pkg/openai/constants.go`。
...
6. Antigravity `web_search` 请求固定使用官方模型表内的 `gemini-3.5-flash-low` 作为兜底发包模型。
7. 计费按最终官方发包模型 `UpstreamModel` 查价；日志仍保留外部显示模型，且该计费口径优先于渠道的 `requested` / `channel_mapped` 配置。
...
| 模型收敛 | 新建账号默认使用官方 8 模型，`/models` 面向客户端仍收敛到官方 8 模型；手动“同步上游支持的模型”保存真实同步结果，不再把未知上游结果改写成官方默认值。 |
...
| 成本 | 按最终官方发包模型计费；`gpt-oss-120b-medium` 按 `$0.05 / $0.01 / $0.20` 每 1M tokens 计价。 |

关键文件：

`backend/internal/pkg/antigravity/official_models.go`、`backend/internal/pkg/antigravity/claude_types.go`、`backend/internal/pkg/antigravity/oauth.go`、`backend/internal/pkg/antigravity/gemini_types.go`、`backend/internal/pkg/antigravity/request_transformer.go`、`backend/internal/service/upstream_models.go`、`backend/internal/service/gateway_service.go`、`backend/internal/handler/gateway_handler.go`、`frontend/src/composables/useModelWhitelist.ts`、`frontend/src/components/account/CreateAccountModal.vue`、`frontend/src/components/account/EditAccountModal.vue`、`frontend/src/components/account/ModelWhitelistSelector.vue`、`backend/resources/model-pricing/model_prices_and_context_window.json`。完整差异文件见第 4.3 节。
...
Plus 版本在上游版本后追加自定义序号，例如 `0.1.146-5`；Git tag 使用 `v0.1.146-5`；GHCR 镜像 tag 使用 `ghcr.io/itv3/sub2apiplus:0.1.146-5`。
...
- 发布基线是官方上游 tag `v0.1.146`。
- 当前分支同步过官方 `main` 的 `e8e23425`；其中包含官方 PR #3768 / #3799 引入的 `batch_image` 批量生图等上游后续代码。
- `batch_image` 不属于 Plus 三项增强的自研范围；审核 Plus 自定义实现时，应优先看 `e8e23425..HEAD` 中 API Key mimic、Antigravity、keeper 和 Plus UI 相关文件。
- `keeper/` 是当前新增源码目录；`.codex-captures/` 是本地抓包样本，`.kilo/` 是本地工具配置，不计入源码清单。

需要完整差异时直接生成：

```sh
git diff --name-only v0.1.146..HEAD
git diff --stat v0.1.146..HEAD
git diff --name-only e8e23425..HEAD
```
...
| Plus UI | `frontend/src/views/admin/ApiKeyMimicSettingsView.vue`、`frontend/src/api/admin/accounts.ts`、`frontend/src/router/index.ts`、`frontend/src/components/layout/AppSidebar.vue`、`frontend/src/components/account/EditAccountModal.vue`、`frontend/src/composables/useModelWhitelist.ts`。 |
...
- OpenAI `/v1/responses/compact` 在 Codex mimic 下仍保持 compact JSON 形态和非流式响应协议。
...
- Antigravity 新建账号默认白名单/映射和 `/models` 仍统一到官方 8 模型，不出现 16 个重复模型；手动“同步上游支持的模型”仍保存真实同步结果，不回退伪造成官方默认列表。
- Antigravity 官方伪装仍保留官方 UA、fixed `thinkingBudget`、`labels.model_enum`、`labels.trajectory_id`、同源 `requestId`，并确保客户端 `thinking` / `output_config.effort` 不覆盖官方 8 模型的上游预算；`web_search` 兜底模型仍在官方 8 模型表内。
- Antigravity 计费仍优先使用最终官方发包模型 `UpstreamModel`，并保留 `gpt-oss-120b-medium` 的 `$0.05 / $0.01 / $0.20` 每 1M tokens。
