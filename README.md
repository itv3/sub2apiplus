...
| 功能 | 定位 | 配置归属 |
|---|---|---|
| API Key 官方客户端兼容 | 让非官方客户端经 sub2apiplus 转发时尽量接近官方 Claude / Codex 客户端请求形态。 | `sub2apiplus` 管理后台和账号 `account.Extra`。 |
...
API Key 官方客户端兼容用于让 Kilo / Cline / Cursor / Roo Code 等非官方客户端经 sub2apiplus 转发到第三方中转站 API 时，尽量接近“Claude / Codex 官方客户端直连该中转站 API”的 header、body、TLS 和路由形态。Codex / Claude 官方客户端接入时会跳过 Plus mimic 增强，按普通 API Key 转发逻辑处理。

核心规则：

1. 仅 Anthropic / OpenAI 的 API Key 账号生效，不改变 OAuth 账号逻辑。
2. 仅非官方客户端触发 mimic；检测到 Codex / Claude 官方客户端（桌面版和 CLI）时跳过 mimic。
3. mimic 与 passthrough 运行时互斥；同时开启时，非官方客户端优先走 mimic。
4. 测试连接仍走官方客户端请求形态：OpenAI 复用 Codex mimic profile，Anthropic 构造 Claude Code 风格请求。
5. 官方客户端跳过 mimic 后不适用 mimic 身份头保护，按普通 API Key 逻辑处理；非官方客户端命中 mimic 时，关键伪装 header 不允许被账号级 header override 覆盖。
6. OpenAI Codex mimic 命中后，`/v1/messages` 也必须走 Responses mimic 链路，不受 `force_chat_completions`、普通 Responses probe false 或 `openai_responses_supported=false` 影响。
7. OpenAI API Key Codex mimic 当前只支持 HTTP/SSE 出站：非官方客户端命中 mimic 时强制使用 HTTP，不进入 Responses WebSocket 上游；官方 Codex 客户端跳过 mimic 后，按普通 API Key 账号的全局 WS 开关、账号 WS 开关、force HTTP 和 WSv2 mode 正常选择 HTTP 或 WebSocket。该请求级判断必须同时作用于账号调度、粘性账号复核和最终转发。
...
| 平台 | mimic 目标 | 当前行为 |
|---|---|---|
| Anthropic API Key | Claude Code | 非官方客户端独立处理 `/v1/messages` 和 `count_tokens`，补 Claude Code header / beta / TLS / body 形态（UA 基线 `claude-cli/2.1.207 (external, sdk-cli)`，`anthropic-beta` 对齐官方 CLI 完整列表，`claude-fable-5` 等大上下文模型按需追加 `context-1m`），做已知工具名归一和响应侧结构化回写；官方 Claude 客户端跳过。 |
| OpenAI API Key | Codex | 非官方客户端按 `desktop_0_142` 默认模拟官方 Codex（基线 `codex_exec/0.144.1`，Linux/aarch64 形态，补 `x-openai-internal-codex-responses-lite`、`x-codex-*` 身份头和 turn metadata）；`cli_rs_0_125` 可回滚旧 Codex CLI。Responses 路由、body 默认值（含 `store=false`、`stream`、`include=reasoning.encrypted_content`、`reasoning.context=all_turns`、`text.verbosity=low`）、`prompt_cache_key`、`client_metadata`、TLS profile 和 capability probe 都按 mimic profile 处理；官方 Codex 客户端跳过。 |
...
- 不承诺 100% 复制官方客户端；服务端隐藏 prompt、账号侧状态、产品侧 memory、UI 上下文都可能超出 sub2apiplus 能力范围。Codex mimic 已按 ALPN 走真正的 HTTP/2 wire，但 HTTP/2 帧级指纹（SETTINGS 参数、伪 header 顺序、window 更新等，当前由 Go `x/net/http2` 生成）尚未逐字节对齐官方 Rust h2 栈。
...
1. `model_mapping` 缺失或为空时，为兼容旧账号，默认允许并展示官方抓包确认的 8 个模型；新建账号仍默认写入这 8 个模型的自映射，管理员可在账号编辑页通过“自定义模型名称”手动追加新模型。
...
5. 存在非空显式 `model_mapping` 时，只有规范化后满足 `model -> model` 的自映射项构成账号允许发包的模型集合；官方模型不会被隐式补回，因此管理员可以通过取消自映射将官方模型从该账号白名单和 `/models` 中移除。普通映射、通配符和历史兼容别名只负责解析请求，最终目标必须命中该允许集合。
6. Antigravity `web_search` 请求固定使用默认官方模型表内的 `gemini-3.5-flash-low` 作为最终发包模型；无显式白名单时由官方默认集合允许，存在显式白名单时必须保留该模型的自映射，否则请求按模型不在白名单拒绝，不能通过 `web_search` 绕过白名单。
7. 显式配置但格式无效、且无法解析出有效字符串映射的 `model_mapping` 按空白名单处理，不回退到官方默认集合，避免配置错误意外放开模型。
8. 计费按最终实际发包模型 `UpstreamModel` 查价；日志仍保留外部显示模型，且该计费口径优先于渠道的 `requested` / `channel_mapped` 配置。
...
9. 只有当前可调度且具备有效平台 API Key 凭据的 OpenAI / Anthropic API Key 账号才进入 keeper 候选列表。停用、手动不可调度、过期、过载、限流、临时不可调度或配额耗尽的账号不会返回给 keeper，也不会为其签发 scoped proxy token；账号恢复可调度后会在后续扫描中自动重新进入候选列表。
10. keeper 账号代理入口在实际执行前再次检查账号可调度状态，防止候选列表返回或 scoped token 签发后账号状态发生变化。

保活配置放在“Plus 增强功能 / 账号保活”分组中：

| 配置 | 说明 |
|---|---|
| 保活账号 | 从当前可调度且具备有效凭据的 OpenAI / Anthropic API Key 账号中选择；账号后续变为不可调度时会暂时退出 keeper 候选，恢复后自动重新进入。 |
...
| 范围 | 入口文件 |
|---|---|
| API Key mimic | `backend/internal/service/account_apikey_mimicry.go`、`gateway_apikey_mimicry.go`、`apikey_mimic_identity.go`、`openai_apikey_mimic_profile.go`、`openai_apikey_mimicry.go`、`openai_codex_transform.go`、`openai_gateway_service.go`、`openai_upstream_http.go`、`openai_ws_protocol_resolver.go`、`openai_account_scheduler.go`、`openai_ws_forwarder_ingress.go`、`openai_ws_forwarder_support.go`、`gateway_tool_rewrite.go`、`account_header_override.go`、`identity_service.go`、`backend/internal/pkg/claude/constants.go`、`backend/internal/pkg/tlsfingerprint/dialer.go`、`backend/internal/pkg/tlsfingerprint/h2.go`、`backend/internal/repository/http_upstream.go`，以及 `backend/internal/handler/openai_gateway_handler.go`。 |
| Antigravity | `backend/internal/pkg/antigravity/official_models.go`、`request_transformer.go`、`backend/internal/service/antigravity_gateway_service.go`、`antigravity_gateway_claude.go`、`antigravity_models_advertised.go`、`upstream_models.go`、`model_rate_limit.go`、`backend/resources/model-pricing/model_prices_and_context_window.json`。 |
...
- Anthropic API Key mimic 只对非官方客户端触发；官方 Claude 客户端应跳过 mimic，必要时回到 passthrough / 普通 API Key 逻辑。
- Anthropic API Key mimic 的 `anthropic-beta` 走 mimic 专用完整列表（对齐官方 CLI 抓包），不复用普通 API Key 的精简 `APIKeyBetaHeader`；普通 / 官方 API Key 直连路径的 beta 保持不变。
- Anthropic 工具名归一和 per-request reverseMap 仍只改结构化工具字段。
- 非官方客户端命中 API Key mimic 时仍保护官方身份头，账号级 header override 不能覆盖 Codex / Claude Code 关键头。
- OpenAI API Key mimic 只对非官方客户端参与 `isCodexCLI` 和 Responses 路由判定；官方 Codex 客户端应跳过 mimic。
- OpenAI 非官方客户端命中 API Key mimic 时仍强制 HTTP；官方 Codex 客户端跳过 mimic 后，WS ingress 的账号调度、previous response 粘连复核和最终转发都必须恢复普通 API Key WS/HTTP 路由，不能只在请求 body/header mimic 层跳过。
...
- `desktop_0_142` 仍是默认 Codex profile（基线 `codex_exec/0.144.1`），TLS fingerprint 未显式绑定时不回退到 Node.js profile。
- Codex 默认 TLS profile 仍复刻 `codex_exec/0.144.1` 抓包（ALPN `h2, http/1.1`、`X25519MLKEM768` 混合组、无 GREASE 的 11 extension 顺序），且 ALPN 含 h2 时经 `x/net/http2` 走真正 HTTP/2 wire，不被 net/http 退化成 h1.1；`cli_rs_0_125` 仍回滚到 Node.js 风格 h1 profile。
- Responses capability probe 仍按 mimic / 非 mimic 分键保存。
- `CodexBaseInstructionsForModel()` 的 `gpt-5.5` / `gpt-5.2` 映射仍符合当前策略；`gpt-5.6` 等未单独维护 prompt 的版本仍回退到最新版本（当前 GPT-5.5）。
...
- Antigravity 显式白名单只由原始 `model_mapping` 的自映射项构成；请求校验与 `/models` 广告必须使用同一允许集合。显式移除的官方模型不得被默认模型表、兼容别名、通配符或 `web_search` 兜底重新放开。
...
- Plus 增强功能入口仍包含 API Key 官方客户端兼容、Antigravity 增强和账号保活。
...
- keeper 候选列表只返回当前可调度且具备有效凭据的 OpenAI / Anthropic API Key 账号；账号代理入口仍在执行前二次检查可调度状态，临时不可调度账号恢复后可自动重新进入候选列表。
