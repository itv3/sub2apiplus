...
| 版本基线 | 当前 Plus 版本为 `0.1.155-3`（见 `backend/cmd/server/VERSION`，发版后以最新 tag 为准）；已合并上游 tag `v0.1.155`，自定义差异优先看 `v0.1.155..HEAD`。 |
...
镜像 `ghcr.io/itv3/sub2apiplus` 支持 `linux/amd64` 和 `linux/arm64`；`latest` 为最新稳定版，`<Plus 版本>`（如 `0.1.155-3`）固定版本，`<上游主版本>.<上游次版本>` 指向对应 minor 线的最新补丁。
...
Plus 版本在上游版本后追加自定义序号，例如 `0.1.155-3`；Git tag 为 `v0.1.155-3`，镜像 tag 为 `ghcr.io/itv3/sub2apiplus:0.1.155-3`。Release workflow 使用 annotated tag 的 message 生成 Release notes，因此必须使用 `git tag -a -m`；轻量 tag 不包含说明正文。
