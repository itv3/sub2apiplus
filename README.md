...
| 平台 | mimic 目标 | 当前行为 |
|---|---|---|
| Anthropic API Key | Claude Code | 非官方客户端独立处理 `/v1/messages` 和 `count_tokens`，补 Claude Code header / beta / TLS / body 形态，做已知工具名归一和响应侧结构化回写；官方 Claude 客户端跳过。 |
| OpenAI API Key | Codex | 非官方客户端按 `desktop_0_142` 默认模拟 Codex Desktop；`cli_rs_0_125` 可回滚旧 Codex CLI。Responses 路由、body 默认值、`prompt_cache_key`、`client_metadata`、TLS profile 和 capability probe 都按 mimic profile 处理；官方 Codex 客户端跳过。 |
...
TLS fingerprint 默认内置 Claude CLI / Node.js 风格 profile。API Key 官方客户端兼容需要在账号 `extra` 中启用对应 mimic 开关和 `enable_tls_fingerprint`，具体行为以第 1 节和代码常量为准。
...
| 范围 | 入口文件 |
|---|---|
| API Key mimic | `backend/internal/service/account_apikey_mimicry.go`、`gateway_apikey_mimicry.go`、`apikey_mimic_identity.go`、`openai_apikey_mimic_profile.go`、`openai_codex_transform.go`、`openai_gateway_service.go`、`openai_ws_protocol_resolver.go`、`openai_account_scheduler.go`、`openai_ws_forwarder_ingress.go`、`openai_ws_forwarder_support.go`、`gateway_tool_rewrite.go`、`account_header_override.go`，以及 `backend/internal/handler/openai_gateway_handler.go`。 |
