...
| 最大输出 token | 目前主要通过 `account.Extra.keeper_keepalive_max_output_tokens` 设置，默认 512；主服务内部代理会按该值钳制官方客户端请求，服务端硬上限为 1024。 |
...
| 项 | 规则 | 示例 |
|---|---|---|
| Plus 版本 | 上游版本后追加自定义序号 | `0.1.146-3` |
| Git tag | `v<Plus 版本>` | `v0.1.146-3` |
| GHCR 镜像 | `ghcr.io/itv3/sub2apiplus:<Plus 版本>` | `ghcr.io/itv3/sub2apiplus:0.1.146-3` |
...
backend/internal/pkg/antigravity/official_models.go
...
backend/internal/handler/admin/account_handler_keeper.go
backend/internal/handler/admin/account_handler_keeper_test.go
...
backend/internal/service/account_test_service_keeper.go
