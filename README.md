...
| 版本基线 | 当前 Plus 版本为 `0.1.153-1`（见 `backend/cmd/server/VERSION`，发版后以最新 tag 为准）；已合并上游 tag `v0.1.153`，自定义差异优先看 `v0.1.153..HEAD`。 |
...
### 1.4 上游 v0.1.153 同步能力

本版完整合并上游 `v0.1.153`，并继续保留前述 Plus mimic、Antigravity 和 keeper 行为。主要新增与修复如下：

| 范围 | 更新内容 |
|---|---|
| Grok / xAI | 新增视频编辑和视频延长端点；API Key 账号支持同步上游模型；OAuth 账号可使用经过校验的第三方 API Base URL。 |
| OpenAI WebSocket | 入站连接默认限制为每个 API Key 64 条，并通过 Redis Lease 跨实例协调；跨轮空闲默认 300 秒后正常关闭，避免长连接无限占用资源。 |
| 网关兼容 | 修复池模式重试次数在 Anthropic、Gemini 和通用转发路径不生效的问题；修复 Codex Chat 桥丢失 additional/custom tools、Anthropic 流式停止原因和 Read 工具参数延迟。 |
| 调度与统计 | 修复异常缓存时间阻塞账号调度和用量统计跨时区日期偏移；API Key 最近使用 IP 改为有界索引查询，并增加非事务并发索引迁移。 |
| 管理体验 | OpenAI OAuth 账号可手动覆盖 `plan_type`；账号列表小数据量关闭虚拟化并按行主键缓存高度；补齐中文 overview 和 misc 翻译。 |
| Web 与安全 | 内嵌静态资源增加一年期 immutable 缓存；修复 `/alpha/search` 被 SPA 拦截；删除可能泄露内部渠道配置的废弃支付接口。 |
| Apple 容器 | 新增适用于 Apple silicon、macOS 26 和 Apple `container` 1.1+ 的三服务部署脚本，Docker Compose 仍是生产环境推荐方案。 |
...
镜像 `ghcr.io/itv3/sub2apiplus` 支持 `linux/amd64` 和 `linux/arm64`；`latest` 为最新稳定版，`<Plus 版本>`（如 `0.1.153-1`）固定版本，`<上游主版本>.<上游次版本>` 指向对应 minor 线的最新补丁。
...
Plus 版本在上游版本后追加自定义序号，例如 `0.1.153-1`；Git tag 为 `v0.1.153-1`，镜像 tag 为 `ghcr.io/itv3/sub2apiplus:0.1.153-1`。Release workflow 使用 annotated tag 的 message 生成 Release notes，因此必须使用 `git tag -a -m`；轻量 tag 不包含说明正文。
...
发布基线为上游 tag `v0.1.153`，当前分支已通过合并提交同步；审核 Plus 实现时优先看 `v0.1.153..HEAD` 中 mimic、Antigravity、keeper 和 Plus UI。`batch_image` 属于上游；`keeper/` 是新增源码，`.codex-captures/` 和 `.kilo/` 是本地样本或工具配置，不计入源码清单。完整差异用以下命令生成：

```sh
git diff --name-only v0.1.153..HEAD
git diff --stat v0.1.153..HEAD
```
