...
下载保活项目。`SUB2APIPLUS_KEEPER_PROJECTS` 写的是项目名，目录必须放在 keeper 的 `projects` 下：
...
| `KEEPER_WEB_USERNAME` / `KEEPER_WEB_PASSWORD` | keeper 独立 Web 入口的 Basic Auth；留空时通常只通过 sub2apiplus 后台访问。 |
...
| `POST /api/v1/internal/keeper/run?target=<target-name>` | 立即执行指定保活目标。 |
...
| `ANY /api/v1/internal/keeper/openai/accounts/:id/*` | Codex 官方客户端的 OpenAI 代理入口，使用现有账号上游配置。 |
| `ANY /api/v1/internal/keeper/anthropic/accounts/:id/*` | Claude 官方客户端的 Anthropic 代理入口，使用现有账号上游配置。 |

内部接口优先使用 `SUB2APIPLUS_KEEPER_INTERNAL_TOKEN` 鉴权，keeper 通过 `x-api-key` 或 `Authorization: Bearer` 调用；后台管理员请求仍可走现有 admin auth。接口不能暴露账号密钥、上游 `Authorization`、上游 `x-api-key` 或未脱敏日志。
...
本清单以官方上游 tag `v0.1.144` 为基线，覆盖当前工作树中 Plus / fork 自定义触及的文件。`keeper/` 是当前新增源码目录；`.codex-captures/` 是本地抓包样本，`.kilo/` 是本地工具配置，不计入源码清单。
