...
| 版本基线 | 当前 Plus 版本为 `0.1.152-1`（见 `backend/cmd/server/VERSION`，发版后以最新 tag 为准）；已合并上游 tag `v0.1.152`，自定义差异优先看 `v0.1.152..HEAD`。 |
...
### 1.4 上游 v0.1.152 同步能力

本版完整合并上游 `v0.1.152`，并继续保留前述 Plus mimic、Antigravity 和 keeper 行为。主要新增与修复如下：

| 范围 | 更新内容 |
|---|---|
| Grok / xAI | 支持 OAuth 与 xAI API Key 两类账号；API Key 账号可执行 Responses 转发和连接测试。免费 OAuth 账号支持提示词缓存，可缓存请求自动切换到 Responses 链路。 |
| Grok 稳定性 | 展示剩余配额，配额耗尽时记录限流状态；OAuth 订阅请求统一经 CLI 代理路由；清理 composer 不兼容的推理参数，并按平台细分无可用账号错误。 |
| Codex 网页搜索 | 接入 `alpha/search` 网页搜索端点并支持按次计费；默认单价为 `$0.01/次`，分组可覆盖单价，最终费用叠加分组倍率，非成功响应不计费。 |
| Chat 兼容桥 | 支持 `tool_search` 服务端工具、custom 工具和 MCP namespace 子工具回程还原；仅转发转换后实际存在的 `tool_choice`，工具撞名时明确拒绝请求。 |
| Responses / Anthropic | 补齐普通及流式转换中的 `cache_creation_input_tokens`，恢复缓存创建 token 的准确统计。 |
| Codex 可靠性 | 修复 OAuth Messages 身份头、`remote_compaction_v2` 原生 Responses 链路、续链非法 `item_*` ID、compact 心跳 writer 生命周期及释放后空指针问题。 |
| 管理体验 | Fast/Flex 策略指定用户改为邮箱模糊搜索多选；分组页面增加网页搜索单次价格配置与实时价格预览。 |
...
镜像 `ghcr.io/itv3/sub2apiplus` 支持 `linux/amd64` 和 `linux/arm64`；`latest` 为最新稳定版，`<Plus 版本>`（如 `0.1.152-1`）固定版本，`<上游主版本>.<上游次版本>` 指向对应 minor 线的最新补丁。
...
Plus 版本在上游版本后追加自定义序号，例如 `0.1.152-1`；Git tag 为 `v0.1.152-1`，镜像 tag 为 `ghcr.io/itv3/sub2apiplus:0.1.152-1`。Release workflow 使用 annotated tag 的 message 生成 Release notes，因此必须使用 `git tag -a -m`；轻量 tag 不包含说明正文。
...
发布基线为上游 tag `v0.1.152`，当前分支已通过合并提交同步；审核 Plus 实现时优先看 `v0.1.152..HEAD` 中 mimic、Antigravity、keeper 和 Plus UI。`batch_image` 属于上游；`keeper/` 是新增源码，`.codex-captures/` 和 `.kilo/` 是本地样本或工具配置，不计入源码清单。完整差异用以下命令生成：

```sh
git diff --name-only v0.1.152..HEAD
git diff --stat v0.1.152..HEAD
```
