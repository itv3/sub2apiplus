...
| 最大输出 token | 可通过 `account.Extra.keeper_keepalive_max_output_tokens` 设置，默认 512。 |
...
| 接口 | 用途 |
|---|---|
| `GET /api/v1/internal/keeper/accounts` | 返回已启用保活的账号、模型、prompt、项目、最大输出 token、最近使用时间、下一次时间和 due 判断。 |
