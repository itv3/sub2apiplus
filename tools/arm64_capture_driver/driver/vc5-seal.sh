...
# 幂等：每一步以产物存在与否决定跳过；evidence-manifest.json（seal 预览产物）存在后不再对 evidence/** 做任何写入。
...
if [ ! -f "$EV/client/raw/kilo-facts.json" ]; then
...
if [ ! -f "$EV/environment/client-after/probe-manifest.json" ]; then echo "=== 批次 ${SEQ}：seal checkpoint"; batch "$SEQ" action-plan-vc5-seal-checkpoint.json; SEQ=$((SEQ+1)); fi
...
if [ ! -f "$EV/assertion-bundle/capture-manifest.json" ]; then
...
echo "SEAL_DONE ATT=$ATT SEAL_SHA=$SEAL_SHA NEXT_SEQ=$SEQ"
