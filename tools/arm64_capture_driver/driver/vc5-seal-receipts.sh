...
ID_ARGS=(--campaign-id "$CID" --attempt-id "$ATT" --run-nonce "$RUN_NONCE" --candidate-id "$CAND" --target-version "$TARGET_VERSION" --profile-id "$PROFILE_ID" --profile-digest "$PROFILE_DIGEST" --image-id "$IMAGE_ID" --image-reference "$IMAGE_REF" --source-tree-sha256 "$TREE" --build-id "$BUILD_ID" --deployed-version "$DEPLOYED")
...
FIN_ID=(--campaign-id "$CID" --attempt-id "$ATT" --run-nonce "$RUN_NONCE" --attempt-started-at-utc "$STARTED" --client-checkpoint-at-utc "$CKPT" --candidate-id "$CAND" --target-version "$TARGET_VERSION" --profile-id "$PROFILE_ID" --profile-digest "$PROFILE_DIGEST" --source-tree-sha256 "$TREE" --build-id "$BUILD_ID" --deployed-version "$DEPLOYED")
