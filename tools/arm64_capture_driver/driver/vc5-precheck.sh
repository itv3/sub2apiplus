...
args = argparse.Namespace(campaign_dir=campaign_dir, candidate_id=CAND, runtime_image=f"sub2apiplus-c0154-candidate@{IMAGE_ID}", candidate_image_id=IMAGE_ID, candidate_source=Path(SRC), build_id=BUILD_ID, deployed_version="0.154.0", profile_id=PROFILE_ID, profile_digest=PROFILE_DIGEST, candidate_purpose="production_replacement")
