...
run_id=${RUN_ID:-"official-codex-scenario-$(date -u +%Y%m%dT%H%M%SZ)"}
subject=codex-http
direct_started=0
mitm_started=0
...
cleanup() {
  ...
  if [[ $direct_started == 1 ]]; then
    docker exec "$capture_container" /opt/oauth-capture/scripts/stop_direct.sh "$subject" || true
  fi
...
docker exec "$capture_container" \
  ...
  --mode official-http --scenario "$scenario" --model "$codex_model" \
  --output-dir "/capture/runs/$run_id/result/direct" --timeout 300
...
python3 - "$capture_root/runs/$run_id" "$run_id" "$codex_model" "$scenario" <<'PY'
import hashlib
import json
import os
import sys
...
pcap = root / "direct/codex-http/egress.pcap"
jsonl = sorted(root.glob("mitm/codex-http/*.jsonl"))
payload = {
    "schema_version": "official-codex-scenario-capture/v1",
    "run_id": sys.argv[2],
...
if payload["status"] != "complete" or payload["direct"]["pcap_bytes"] <= 24:
    raise SystemExit("官方 Codex 场景 direct/MITM 校验失败")
if not any(item["records"] > 0 for item in payload["mitm"]["jsonl"]):
    raise SystemExit("官方 Codex 场景 MITM 未记录请求")
