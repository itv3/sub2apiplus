"""冻结 Codex CLI 0.149.1 r19 多级后继递归重放 transition。"""
...
BASE_COMMIT = "0be1e9c115fc5da8ef8b216ed3b8b0521ace5924"
TRANSITION_PATH = (
    ROOT
...
EXPECTED_CONTRACT = {
    "reason": "successor_chain_origin_resolution",
    ...
    "recursive_predecessor_replay_required": True,
    "original_attempt_directory_required": True,
    "relative_path_reinterpretation_allowed": False,
    "account_transition_contract_preserved": True,
}
EXPECTED_SAFETY = {
...
def reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise ValueError(f"r19 后继链 transition 包含重复字段：{key}")
        result[key] = value
    return result
...
def load_transition() -> dict[str, Any]:
    """读取 r19 多级后继 transition。"""

    return load_document(TRANSITION_PATH, "r19 多级后继 transition")


def base_blob(path: str) -> bytes | None:
    """读取 r18 最终提交中的普通 Git blob；不存在时返回 None。"""
...
def validate_transition(document: dict[str, Any]) -> None:
    ...
    if set(document) != {
        ...
        raise ValueError("r19 多级后继 transition 顶层字段非法")
    if (
        document["schema_version"]
        != "official-client-codex-0.149.1-r19-successor-chain-transition/v1"
        or document["base_commit"] != BASE_COMMIT
        or document["scope"] != "codex-0.149.1-r19-successor-chain"
        or document["framework_stage"]
        != "VC-3/VC-4/SAME-VERSION-SUCCESSOR"
        or document["result"] != "successor_chain_replay_tooling_frozen"
        or document["identity_sha256"] != canonical_identity(document)
    ):
        raise ValueError("r19 多级后继 transition 身份非法")
    try:
        datetime.fromisoformat(document["issued_at_utc"].replace("Z", "+00:00"))
    except (AttributeError, ValueError) as error:
        raise ValueError("r19 多级后继 transition 时间非法") from error

    predecessor = load_document(PREDECESSOR_PATH, "r18 后继账号 transition")
    if document["predecessor_transition"] != {
        ...
        raise ValueError("r19 多级后继 transition 前序绑定非法")
    if (
        predecessor.get("schema_version")
        != "official-client-codex-0.149.1-r18-successor-account-transition/v1"
        or predecessor.get("scope") != "codex-0.149.1-r18-successor-account"
        or predecessor.get("result")
        != "successor_account_transition_tooling_frozen"
        or predecessor.get("identity_sha256") != canonical_identity(predecessor)
    ):
        raise ValueError("r19 多级后继 transition 前序身份非法")
    if document["successor_chain_contract"] != EXPECTED_CONTRACT:
        raise ValueError("r19 多级后继递归合同非法")
    if document["safety"] != EXPECTED_SAFETY:
        raise ValueError("r19 多级后继安全边界非法")

    entries = document.get("transitions")
    if not isinstance(entries, list):
        raise ValueError("r19 多级后继 transition 文件闭集非法")
    ...
    if (
        ...
        raise ValueError("r19 多级后继 transition 路径未排序、重复或越界")
    ...
    if document.get("path_set_sha256") != expected_path_set:
        raise ValueError("r19 多级后继 transition 路径摘要不一致")
    ...
    for entry in entries:
        if set(entry) != {
            ...
            raise ValueError("r19 多级后继 transition 条目字段非法")
        path = entry["path"]
        current = ROOT / path
        ...
        if (
            ...
            raise ValueError(f"r19 多级后继 transition 条目非法：{path}")


@lru_cache(maxsize=1)
def load_validated_transition() -> dict[str, Any]:
    """读取并完整重放 r19 transition。"""
...
def r19_supersedes(path: str, prior_digest: str, current_digest: str) -> bool:
    """只承认 r19 收据登记的精确 path/from/to 三元组。"""
...
transition_supersedes = r19_supersedes


class Codex01491R19SuccessorChainTransitionTest(unittest.TestCase):
    ...
    def test_transition_精确后继三元组被承认(self) -> None:
        ...
        self.assertFalse(
            r19_supersedes(entry["path"], "0" * 64, entry["to_sha256"])
        )
