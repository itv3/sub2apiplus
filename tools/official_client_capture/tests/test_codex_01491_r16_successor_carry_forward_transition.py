"""冻结 Codex CLI 0.149.1 r16 同版本后继承接 transition。"""
...
BASE_COMMIT = "972c7e6e3fd2e9577cb8d1a3e7660b776d8a6ecd"
TRANSITION_PATH = (
    ROOT
...
def reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise ValueError(f"r16 后继承接 transition 包含重复字段：{key}")
        result[key] = value
    return result
...
def load_transition() -> dict[str, Any]:
    """读取 r16 同版本后继承接 transition。"""

    return load_document(TRANSITION_PATH, "r16 后继承接 transition")


def base_blob(path: str) -> bytes | None:
    """读取基线提交中的普通 Git blob；不存在时返回 None。"""
...
def validate_transition(document: dict[str, Any]) -> None:
    ...
    if set(document) != {
        ...
        raise ValueError("r16 后继承接 transition 顶层字段非法")
    if (
        document["schema_version"]
        != "official-client-codex-0.149.1-r16-successor-carry-forward-transition/v1"
        or document["base_commit"] != BASE_COMMIT
        or document["scope"] != "codex-0.149.1-r16-successor-carry-forward"
        or document["framework_stage"] != "VC-3/VC-4/SAME-VERSION-SUCCESSOR"
        or document["result"] != "successor_carry_forward_tooling_frozen"
        or document["identity_sha256"] != canonical_identity(document)
    ):
        raise ValueError("r16 后继承接 transition 身份非法")
    try:
        datetime.fromisoformat(document["issued_at_utc"].replace("Z", "+00:00"))
    except (AttributeError, ValueError) as error:
        raise ValueError("r16 后继承接 transition 时间非法") from error

    predecessor = load_document(PREDECESSOR_PATH, "r15 Formal 分类 transition")
    if document["predecessor_transition"] != {
        ...
        raise ValueError("r16 后继承接 transition 前序绑定非法")
    if (
        predecessor.get("schema_version")
        != "official-client-codex-0.149.1-r15-formal-classification-transition/v1"
        or predecessor.get("scope") != "codex-0.149.1-r15-formal-classification"
        ...
        raise ValueError("r16 后继承接 transition 前序身份非法")
    if document["carry_forward_contract"] != EXPECTED_CONTRACT:
        raise ValueError("r16 后继承接合同非法")
    if document["safety"] != EXPECTED_SAFETY:
        raise ValueError("r16 后继承接安全边界非法")

    entries = document.get("transitions")
    if not isinstance(entries, list):
        raise ValueError("r16 后继承接 transition 文件闭集非法")
    ...
    if (
        ...
        raise ValueError("r16 后继承接 transition 路径未排序、重复或越界")
    ...
    if document.get("path_set_sha256") != expected_path_set:
        raise ValueError("r16 后继承接 transition 路径摘要不一致")
    ...
    for entry in entries:
        if set(entry) != {
            ...
            raise ValueError("r16 后继承接 transition 条目字段非法")
        path = entry["path"]
        current = ROOT / path
        ...
        if (
            ...
            raise ValueError(f"r16 后继承接 transition 条目非法：{path}")

    receipt_schema = load_document(
        ROOT
    ...
    if (
        ...
        .get("schema_version", {})
        .get("const")
        != "codex-upgrade-predecessor-import/v1"
    ):
        raise ValueError("r16 前序导入 Schema 身份非法")


@lru_cache(maxsize=1)
def load_validated_transition() -> dict[str, Any]:
    """读取并完整重放 r16 transition。"""
...
def r16_supersedes(path: str, prior_digest: str, current_digest: str) -> bool:
    """只承认 r16 收据登记的精确 path/from/to 三元组。"""
...
transition_supersedes = r16_supersedes


class Codex01491R16SuccessorCarryForwardTransitionTest(unittest.TestCase):
    ...
    def test_transition_精确后继三元组被承认(self) -> None:
        ...
        self.assertFalse(
            r16_supersedes(entry["path"], "0" * 64, entry["to_sha256"])
        )
