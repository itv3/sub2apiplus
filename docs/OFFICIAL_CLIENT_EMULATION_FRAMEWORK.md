...
受管工具身份由客户端指南登记的显式策略分层：产出侧（wire producer）、证据语义侧、控制侧与忽略集合，
并为策略文件本身计算独立摘要；整树摘要只作溯源与副本一致性。只有产出侧身份变化才是重采判据，且必须
经两阶段过渡（先冻结受影响 Job 闭集并批准，再以补跑结果封存过渡）；证据语义变化只在评估前追加单调的
评估纪元链；控制侧变化只重跑控制门禁；策略变化必须升级策略版本并经兼容收据与激活认证后以新 Campaign
承接。策略无法解析时计划失败关闭。历史 attempt 执行时的产出侧身份由部署回滚副本按策略重算裁定，无副本
时只有整树相等才算相等。
...
经故障夹具与相关真实链验证的最小修复受控部署后，从原 Campaign 的合法 checkpoint 继续；
保留仍可信且依赖未变的结果，并记录实际执行、复用及新增请求数量。
...
| 环境面 | 平台、架构、主机／容器、路由、出口、MTU、磁盘和依赖事实 | 重新核验当前环境，按版本化真实依赖投影决定直接下游；已授权的出口变化本身不使历史结果失效 |
...
出口选择通过受限运维策略显式授权，工具不得写死服务商、IP、监控 Peer 数或链路参数，也不得自动学习当前出口。
源宿主与出口宿主共同强制业务路径，覆盖所有网卡、DNS、代理、IPv4／IPv6、新旧连接及重建首包；更新无放行空窗，
守护停止保留保护并使租期失效。每个受保护容器独立验证，单容器故障单独闭锁，共享故障全部闭锁；任一异常暂停升级
派发并中止进行中的采集，记录原因和影响窗口。修复后必须重新准入并完成受影响 Job 对账，才从合法 checkpoint 恢复。

当前运行准入与历史证据真实性分别检查。用户正常改选出口后无需改工具、重签工具发布认证或重建 Campaign；旧收据
先按原 producer 的完整合同重放，再用版本化投影比较镜像、客户端、账号／模型、抓包拓扑、代理／CA 等真实依赖。
出口及其派生摘要不进入等价身份，完整事实摘要继续防篡改；未知 producer、证明不足或内容变化仍拒绝。修复后的出口
状态不能追认故障窗口内的请求。可信已完成结果继续复用，新目标版本首次官方取证仍重新采集。
...
客户端指南若定义了原子 VC-0 收口入口，Formal Campaign 创建、阶段事件和首批派发必须由该入口在同一
进程内完成；不得用人工远程多命令拼接跨越交接窗口。
...
4. 阶段只能消费前序阶段的封存输出。阶段完成事件、批次编译、checkpoint、具体字段和 runner 均由客户端
   指南定义，但不得依赖人工补派、遍历无关历史证据或用后续结果补写前序事实。客户端指南定义后继批次
   原子入口时，编译与父 runner 创建必须处于同一互斥区；编译完成但父 runner 尚未创建即失败的批次只能
   留下零请求停线事实，不得成为恢复后继输入。
...
收据，不回写 attempt、不新增 attempt 状态枚举、不伪造事件，自身模型请求为零。对账固定顺序为：Campaign
侧收据与账本失败事件 → 一个 outbox batch 承载一个项目事件（请求账务与根因）→ 推入项目总账 → 锁内重放
→ 判定 → 可恢复或永久停线。判定只依据重放后的总账：总账 blocked 或本次账务未决即永久停线；身份不变、
环境已恢复或可恢复、Campaign 与项目 deadline 未到、剩余请求预算大于 0 且同根因累计未达上限才可恢复。
...
时间账本从准备工作的第一项开始连续记录，不得在正式取证、新 Candidate 或恢复执行时重新起算。项目、
Campaign 或阶段预算到期先暂停，保留结果与原阶段状态；暂停期间禁止派发、恢复、复用与封存。三层延期
只能通过绑定两账 head 的批准收据分别追加，累计消耗不清零，原始截止字段不改写。两账中断须按同一批准
补齐后才放行，延期后仍复验既有准入与证据边界。72 小时无新延期批准只提醒复核，只有显式放弃才因预算
进入终态；完整性、请求预算和根因上限的停线规则不变。不得为同一工作对象新建 Campaign、Candidate 或
控制收据来重置计时。
...
预算分两层：Campaign 账本记录本 Campaign 的阶段、attempt、停线与本地请求量；项目总账在升级项目级冻结
绝对截止时间、请求预算、同根因重试上限、根因编码摘要与估计政策，按统一计量单位（模型端点的实际请求）
