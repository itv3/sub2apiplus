# Codex CLI 0.149.1 候选规则画像

> **状态**：`DOC-PRE／preflight-only` 目标输入；尚未创建正式 Campaign，当前 production active 仍为 0.147.0。
> **边界**：本文只冻结 0.149.1 的目标规则与源码锚点，不修改 ReleaseCatalog、Runtime Selector 或生产部署。

# 第二部分 Codex CLI 客户端规则画像

本部分定义规则成立所需的证据标准、观测边界和 0.149.1 目标的 53 个编号项。它回答“什么
...
`e24fb784c7d71140d67afb620f56e9137496cf7f6c9e19217fa3666dcf306278`。私有归档中的候选画像为
`codex-0.149.1-official-r1491-v1`，摘要为
`8e59b38e2ad90a1fd4eb7520c2c54f01fc62f802690d45a2cdab5f91f249fb60`；Release graph 与
Snapshot catalog 候选摘要分别为
`0187b418a0952584cc7b201ed6f0f442c11c5e6b0d69707bda884c0b11ce2f23`、
`769860c88b600c875ff4b670e0b0e99d77ac0dfe40effa6a29d27c6bdb7a6f7b`。0.149.1 的 HTTP、WS
...
证明；它们仍是当前 0.147.0 production active 的历史晋升与激活事实，不能证明 0.149.1 已被晋升、
激活或部署。各规则保留的早期 run ID 是未变化规则的原始证据，不代表目标规则已通过新 Campaign。
...
本文件不声明 0.149.1 已被验收、晋升或激活；正式状态只由 Framework §5.3 的 Campaign、ApprovalFact、AcceptanceFact 与 §5.6 收据证明。
