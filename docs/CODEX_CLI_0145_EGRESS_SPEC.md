...
- **一、背景与任务** —— 为什么做、怎么做
- **二、规则** —— 官方形态是什么样，52 条
- **三、方案** —— 并列两种方案：**方案甲**（Go 内薄层定型，当前在用）与
  **方案乙**（同源 sidecar，已 PoC 未实施），以及两者的对比
...
| 维度 | 方案甲 | 方案乙 |
|---|---|---|
| 一次性投入 | 已付清（本轮完成） | 架构改造：IPC、跨进程 SSE 背压、代码迁移、多语言维护能力 |
| 每次官方升级 | **重新抓包 + 重新对齐**，且源码正在变得不可得 | 跟依赖版本号 |
| 故障面 | 单进程 | 新增一整类跨进程故障（僵死、socket 断、进程回收） |
| 排查难度 | 一份日志、pprof 直接上 | 跨语言，堆栈断裂，需请求 ID 串联 |
| 与上游合并 | 冲突面小（伪装逻辑几乎全在 fork 独有文件） | `go.mod` 与 Dockerfile 冲突面更大 |

### 3.3.3 决策尚未做出

**这是一道"跟多久"的题，不是技术题。** 若只需当下对齐一次，方案甲已经够用且成本
已付清；若要长期跟随官方版本，方案乙把每次升级的逆向成本换成了一次性的架构成本。

两条路**并不互斥**：方案甲是当前生产形态，方案乙可作为独立分支推进，验证充分后
再切。本规格表对两者都适用——方案甲用它来实现，方案乙用它来验证。
