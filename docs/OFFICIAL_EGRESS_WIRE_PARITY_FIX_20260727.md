...
## 0. 一致性现状总表（2026-07-27 收盘）

> **官方形态的唯一事实源是 [Codex CLI 0.145.0 出站形态规格表](CODEX_CLI_0145_EGRESS_SPEC.md)。**
> 本表记的是「我们改了什么」，规格表记的是「官方是什么样」。两者冲突时以规格表为准。

本节是全文的索引与状态快照。**先读这里**，细节再按编号跳转到 §3、§6。

### 0.1 已修复

| 编号 | 内容 | 验证强度 |
|---|---|---|
| P0-1 | h1 header 名改全小写（含抑制 Go 默认 UA） | **wire 实测**（h1 探针，17 头中 15 小写） |
| P0-2 | compact 请求不再 zstd 压缩 | 单测 |
| P0-3 | models 请求改走官方 TLS 画像 | **direct pcap**（3 个 ClientHello 同画像） |
| P0-4 | 未知顶层字段改白名单剔除 | **实测**（含自造探针字段全部剔除） |
| P1-5 | `count_tokens` 纳入画像、剥宿主头 | 代码级 |
| P1-6 | images 纳入画像、删 `OpenAI-Beta` | 代码级 |
| P1-7 | `alpha/search` 两条出站纳入画像 | 代码级 |
| P2-9 | 入站判定统一为 strict 口径 | 单测 |
| P2-11 | CC/Messages 入口补能力预热 | 单测 |
| P2-12 | passthrough 清理清单对齐长清单 | 单测 |
| P3-13 | WS turn-state 改从 `response.metadata` 事件提取 | 代码级（**无 wire 证据**，上游未下发该事件） |
| P3-14（部分） | `MAX_HEADER_LIST_SIZE` 10MB → 16KB | **wire 实测**（h2 探针） |
| P3-16（部分） | WS 握手头大小写恢复 tungstenite 形态 | **wire 级单测** |

### 0.2 待修复

| 编号 | 内容 | 为什么还没做 | 影响路径 |
|---|---|---|---|
| P3-15 | h1 header 顺序 + `Host` 大小写与位置 | 须自写 h1 请求侧字节，Go 无配置入口（§6.8） | **直连，默认主路径** |
| P3-16（剩余） | WS 握手头**顺序**仍为 Go 字典序 | 同上，与 P3-15 是同一个工程 | WS |
| P3-14（剩余） | `INITIAL_WINDOW_SIZE`、首个 `WINDOW_UPDATE`、伪头顺序 | utls 与 net/http 的 h2 升级路径不兼容，须 fork（§6.10） | **仅代理** |
| P1-8 | `/v1/live` + WS sideband 缺 `x-codex-beta-features` 等 | 无技术阻塞，是 P1 组唯一跳过的一条 | 需开 `AllowLive` |
| P3-17 | WS 连接池规模与官方单连接模型不同 | 属容量设计，需产品决策 | WS |
| 图像入口映射 | `/v1/images/generations` 转工具调用而非独立端点 | **上游设计，且官方也走这条路**，可不动（§5.1） | images |
| — | `turn_metadata` 缺 `workspaces` 是否为负指纹 | 未复核的观察项 | — |
| P2-10 | `strictIngressIdentity` 按入站 UA 分叉 | 既定决策保留 | — |

### 0.3 三条必须记住的验证限制

1. **MITM 抓包看不到 h1 线形**：经代理必为 h2，HPACK 强制小写并重排，header 大小写与
   顺序在所有 MITM 证据中不可见。h1 差异只能靠 h1 探针（§6.5.2、§6.7）。
2. **mitmproxy 看不到 h2 帧层**：它用自己的 h2 栈重建连接，客户端原始 SETTINGS 的集合、
   取值与帧内顺序在转发后已丢失。须用 CONNECT 代理式 h2 探针（§6.10）。
3. **官方并非处处小写**：普通 HTTP（hyper）全小写，但 WS 握手（tungstenite）前 5 项是
   大写驼峰。照"全小写"一刀切会制造新偏离——本轮已发生过一次并已修回（§6.9）。
...
原 §6.6 记的四项已随后续轮次消化，此处只保留**至今仍未覆盖**的：

1. **P0-2 的 compact 分支未构造真实请求**——只有单测，没有真实 compact 出站的实证。
2. **P3-13 无 wire 证据**：改用 `response.metadata` 事件提取 turn-state 后，上游在历次
   采集中从未下发该事件，因此这条修复只到代码级。
3. **未重跑 Anthropic 三路径与 Kilo 六组合**：本轮改动集中在 OpenAI 侧，Anthropic 画像
   未开启 `LowercaseHeaders`（Claude Code 是 Node.js，形态未验证），需要单独立项。
4. **Sub2API 自身的 h2 出站未实测**：h2 探针目前只采了官方基线与本地 Go 对照，尚未让
   真实 Sub2API 经探针出站取证（需给账号绑代理指向探针）。

原 §6.6 的另两项已完成，记录于此以免重复排查：P0-1 已由 h1 探针实测（§6.5.2）、
P0-3 已由 direct pcap 实证、P1 四条旁路端点与 P3-13 已在第二轮实施（§5.1）。
